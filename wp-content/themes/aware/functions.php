...
/*-----------------------------------------------------------------------------------*/
if ( is_admin() ) {
    include_once (TEMPLATEPATH . '/functions/update-notice.php');
}
...
function ag_register_js() {
	if (!is_admin()) {
		...
		wp_register_script('jquery', get_template_directory_uri() . '/js/jquery-1.4.4.min.js', 'jquery');
...
function ag_reactive_styles() {	
		 $skeletonUrl =  get_template_directory_uri() . '/css/skeleton.css';
		 
		 /* Register Styles */
		 wp_register_style('skeleton', $skeletonUrl);
		 
		 /*Enqueue Styles */
		 wp_enqueue_style( 'skeleton'); 
}
add_action('wp_print_styles', 'ag_reactive_styles');

function ag_animate_style() {	
		 $animateURL =  get_template_directory_uri() . '/css/animate.css';
		 
		 /* Register Styles */
		 wp_register_style('animate', $animateURL);
		 
		 /*Enqueue Styles */
	     wp_enqueue_style( 'animate'); 
}
add_action('wp_print_styles', 'ag_animate_style');
